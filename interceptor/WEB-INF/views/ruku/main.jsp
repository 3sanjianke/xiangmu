  ...
  <script type="text/javascript">
      $(function(){  	  
    	  $("#name_btn").click(function(){
	    		 ...
	    		 $.ajax({
	    			 url:"${APP_PATH}/ruku/selectByKey.do",
	    			 type:"post",
	    			 data:"shopName="+text,
    	  ...
    	  $("#lianhe_btn").click(function(){
	    		 ...
	    		 $.ajax({
	    			 url:"${APP_PATH}/ruku/selectByKey.do",
	    			 type:"post",
	    			 data:"shopName="+text1 + "&shopFenlei="+text2 + "&rukuData="+text3,
	    			 success:function(result){
	    				 if(result.code==100){
      ...
      function zhizuobiaogeByKey(result,msg){
	    	...
	    	//清空之前的信息
 			...
 			$.each(list,function(i,item){
 				var rukuId = item.rukuId;
 				var shopName = item.shopName;
 				var shopFenlei = item.shopFenlei;
 				var rukuShuliang = item.rukuShuliang;
 				var rukuData = item.rukuData;
 				//创建标签
 				var Tr = $("<tr></tr>");
 				var rukuIdTd = $("<td></td>").append(rukuId);
 				var shopNameTd = $("<td></td>").append(shopName);
 				var shopFenleiTd = $("<td></td>").append(shopFenlei);
 				var rukuShuliangTd = $("<td></td>").append(rukuShuliang);
 				var rukuDataTd = $("<td></td>").append(rukuData);
 				
                  
 				$("#td").append(Tr).append(rukuIdTd).append(shopNameTd).append(shopFenleiTd).append(rukuShuliangTd).append(rukuDataTd);
 			});
 			//分页信息
       ...
       <ul id="myTab" class="nav nav-tabs">
		    ...
		    <li class="active"><a href="#see" data-toggle="tab" style="font-size: 14px;"><strong>入库记录</strong></a></li>
	        <li><a href="#add" data-toggle="tab" style="font-size: 14px;"><strong>商品入库</strong></a></li>
		</ul>
		    ...
		    <div class="tab-pane fade" id="add">
		        ...
		        <form class="form-horizontal" id="form1">
					  <div class="form-group">
					    <label for="inputEmail3" class="col-sm-2 control-label">入库单编号</label>
					    <div class="col-sm-5">
					      <input type="text" class="form-control" id="rukuId" placeholder="编号" name="rukuId">
					    </div>
					  </div>
					  <div class="form-group">
					   ...
					   <div class="form-group">
					    <label for="inputEmail3" class="col-sm-2 control-label">入库数量</label>
					    <div class="col-sm-5">
					      <input type="text" class="form-control" id="rukuShuliang" placeholder="备注" name="rukuShuliang">
					    </div>
					  </div>
					   <div class="form-group">
					    <label for="inputEmail3" class="col-sm-2 control-label">入库日期</label>
					    <div class="col-sm-5">
					      <input type="text" class="form-control" id="rukuData" placeholder="备注" name="rukuData">
					    </div>
					  </div>
					   <div class="form-group">
		...
		<script type="text/javascript">
		     ...
		     $("#tijiao").click(function(){
		    	 alert("保存数据");
		    	 $.ajax({
		    		 url:"${APP_PATH}/ruku/insert.do",
		    		 type:'post',
		    		 data:$("#form1").serialize(),
		    		 success:function(result){
		    			 if(result.code==100){
		    				 alert("添加成功");
		    				 window.location.href="${APP_PATH}/ruku/main.do";
		    			 }else{
		     ...
		     **/
		     ...
		     $("#shopName").change(function(){
		    	   ...
		    	   $.ajax({
		    		   url:"${APP_PATH}/kucun/selectFenlei.do",
		    		   type:"post",
