  ...
  <tr>
    ...
    <td width="" align="right">
      ...
      <input type="text" id="paramsOrders.real_name" name="real_name" value="${paramsOrders.real_name}" class="inputstyle" Style="width:100px;"/>&nbsp;
            物品名称：
      <input type="text" id="paramsOrders.goods_name" name="goods_name" value="${paramsOrders.goods_name}" class="inputstyle" Style="width:100px;"/>&nbsp;
            订单状态：
      <select id="orders_flag" name="orders_flag" class="selectstyle" style="width:100px;">
      	<option value="0">请选择</option>
      	<option value="1" ${paramsOrders.orders_flag==1?'selected':''}>待取货</option>
      	<option value="2" ${paramsOrders.orders_flag==2?'selected':''}>已取货</option>
      	<option value="3" ${paramsOrders.orders_flag==3?'selected':''}>用户归还</option>
      	<option value="4" ${paramsOrders.orders_flag==4?'selected':''}>确认归还</option>
      	<option value="5" ${paramsOrders.orders_flag==5?'selected':''}>已作废</option>
      </select>&nbsp;&nbsp;
...
<table width="95%" align="center" class="table_list" cellpadding="0" cellspacing="0">
   ...
   <tr class="list0" onmouseover="tr_mouseover(this)" onmouseout="tr_mouseout(this)"> 
     ...
     <td width="" align="center">
       <a href="Admin_queryOrders.action?orders_id=${orders.orders_id}">查看</a>&nbsp;
       <c:if test="${orders.orders_flag==1}">
       <a href="Admin_sendOrders.action?orders_id=${orders.orders_id}">取货</a>
       </c:if>
       <c:if test="${orders.orders_flag==3}">
       <a href="Admin_confirmOrders.action?orders_id=${orders.orders_id}">确认归还</a>
       </c:if>
     </td>
   </tr> 
   </c:forEach>
